...
# korbinian90@gmail.com slight modification to support writing to mmapped files
...
NIVolume(header::NIfTI1Header, extensions::Vector{NIfTI1Extension}, raw::AbstractArray{T,N}) where {T<:Number,N} =
    NIVolume{typeof(one(T)),N,typeof(raw)}(header, extensions, raw)
NIVolume(header::NIfTI1Header, raw::AbstractArray{T,N}) where {T<:Number,N} =
    NIVolume{typeof(one(T)),N,typeof(raw)}(header, NIfTI1Extension[], raw)
...
# function to copy the the header
Base.copy(x::NIfTI1Header) = NIfTI1Header([getfield(x, k) for k ∈ fieldnames(NIfTI1Header)]...)
...
"""
function niread(file::AbstractString; mmap::Bool=false, write::Bool=false)
    if write
        opentype = "r+"
    else
        opentype = "r"
    end
    file_io = open(file, opentype)
    ...
    elseif header.magic == NI1_MAGIC
        ...
        volume_io = open(volume_name, opentype)
